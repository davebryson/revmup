...
impl Context {
    ...
    fn expand_input_params(&self, fun: &Function) -> Result<Vec<(TokenStream, TokenStream)>> {
        types::expand_params(&fun.inputs, |p| {
            self.internal_structs
                .get_function_input_struct_type(&fun.name, &p.name)
        })
    }
    ...
    fn expand_output_params(&self, fun: &Function) -> Result<Vec<(TokenStream, TokenStream)>> {
        types::expand_params(&fun.outputs, |p| {
            p.internal_type.as_deref().and_then(|s| {
                self.internal_structs
                    .get_function_output_struct_type(&fun.name, s)
            })
        })
    }
    ...
    fn expand_function(
        ...
        let doc_str = format!(
            "Calls the contract's `{name}` (0x{}) function",
            hex::encode(selector)
        );
        ...
        Ok(quote! {
            #[doc = #doc_str]
            pub fn #function_name(&self #function_params) -> ::revmup_contract::call::ContractCall<M, #outputs> {
        ...
        for functions in all_functions.values() {
            ...
            fn name_conflicts(idx: usize, diffs: &[(usize, Vec<&Param>, &&Function)]) -> bool {
                ...
                for (_, other, _) in diffs.iter().filter(|(i, _, _)| *i != idx) {
                    ...
                    if a.iter().all(|d| {
                        b.iter()
                            .any(|o| o.name.to_snake_case() == d.name.to_snake_case())
                    }) {
                        return true;
                    }
                }
