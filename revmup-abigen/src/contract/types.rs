...
pub(crate) fn expand_event_inputs(
    ...
    event
        ...
        .map(|(index, input)| {
            ...
            expand_event_input(input, &event.name, index, internal_structs).map(|ty| {
                (
                    util::expand_input_name(index, &input.name),
                    ty,
                    input.indexed,
                )
            })
...
fn expand_resolved<'a, 'b, F: Fn(&'a Param) -> Option<&'b str>>(
    ...
    match kind {
        ...
        ParamType::FixedArray(ty, size) => Ok(array(
            expand_resolved(ty, param, resolve_tuple)?,
            Some(*size),
        )),
