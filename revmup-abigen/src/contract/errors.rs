...
impl Context {
    ...
    pub(crate) fn errors(&self) -> Result<TokenStream> {
        ...
        let errors_enum_decl = if data_types.len() > 1 {
            Some(self.expand_errors_enum())
        } else {
            None
        };
    ...
    fn expand_error_params(&self, error: &AbiError) -> Result<Vec<(TokenStream, TokenStream)>> {
        types::expand_params(&error.inputs, |p| {
            p.internal_type
                .as_deref()
                .and_then(|s| self.internal_structs.get_struct_type(s))
        })
    }
    ...
    fn expand_errors_enum(&self) -> TokenStream {
        ...
        let variants = self
            ...
            .map(|err| {
                error_struct_name(
                    &err.name,
                    self.error_aliases.get(&err.abi_signature()).cloned(),
                )
            })
            .collect::<Vec<_>>();
