...
impl Context {
    ...
    fn generate_internal_struct(&self, id: &str) -> Result<TokenStream> {
        let sol_struct = self
            ...
            .structs
            .get(id)
            .ok_or_else(|| eyre!("Struct not found"))?;
    ...
    fn generate_human_readable_struct(&self, name: &str) -> Result<TokenStream> {
        let sol_struct = self
            ...
            .structs
            .get(name)
            .ok_or_else(|| eyre!("Struct `{name}` not found"))?;
        let mut fields = Vec::with_capacity(sol_struct.fields().len());
...
impl InternalStructs {
    ...
    pub fn new(abi: RawAbi) -> Self {
        ...
        for item in abi.into_iter().filter(|item| {
            matches!(
    ...
    pub fn get_function_output_struct_solidity_id(
        ...
        self.outputs
            ...
            .and_then(|outputs| {
                outputs
                    .iter()
                    .find(|s| s.as_str() == struct_type_identifier(internal_type))
            })
            .map(String::as_str)
    }
    ...
    pub fn get_struct_type(&self, internal_type: &str) -> Option<&str> {
        self.rust_type_names
            .get(struct_type_identifier(internal_type))
            .map(String::as_str)
    }
