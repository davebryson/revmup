...
impl MultiAbigen {
    ...
    pub fn apply_filter(&mut self, filter: &ContractFilter) {
        self.abigens
            .retain(|abi| filter.is_match(abi.contract_name.to_string()))
    }
...
impl MultiExpansion {
    ...
    pub fn from_abigen(abigens: impl IntoIterator<Item = Abigen>) -> Result<Self> {
        let contracts = abigens
            .into_iter()
            .map(|abigen| abigen.expand())
            .collect::<Result<_>>()?;
...
impl MultiBindings {
    ...
    pub fn write_to_module(self, module: impl AsRef<Path>, single_file: bool) -> Result<()> {
        self.into_inner(single_file)
            .write_to_module(module, single_file)
    }
    ...
    pub fn write_to_crate(
        ...
        self.into_inner(single_file)
            .write_to_crate(name, version, lib, single_file)
    }
    ...
    pub fn ensure_consistent_module(
        ...
        self.into_inner(single_file)
            .ensure_consistent_module(module, single_file)
    }
}
...
impl MultiBindingsInner {
    ...
    fn crate_version(&self) -> String {
        self.try_find_crate_version()
            .unwrap_or_else(|_| DEFAULT_ETHERS_DEP.to_string())
    }
    ...
    fn try_find_crate_version(&self) -> Result<String> {
        let cargo_toml = if let Some(root) = self.root.clone() {
            root
        } else {
            std::env::current_dir()?
        }
    ...
    fn append_module_names(&self, mut buf: impl Write) -> Result<()> {
        let mut mod_names: BTreeSet<_> = self
            .bindings
            .keys()
            .map(|name| util::safe_module_name(name))
            .collect();
...
mod tests {
        ...
        T: FnOnce(Context),
    {
        ...
        let console = Abigen::new(
            "Console",
            crate_root
                .join("../tests/solidity-contracts/console.json")
                .display()
                .to_string(),
        )
        .unwrap();
        ...
        let simple_storage = Abigen::new(
            "SimpleStorage",
            crate_root
                .join("../tests/solidity-contracts/SimpleStorage.json")
                .display()
                .to_string(),
        )
        .unwrap();
    ...
    fn can_generate_multi_file_module() {
        run_test(|context| {
            ...
            multi_gen
                ...
                .build()
                .unwrap()
                .write_to_module(&mod_root, single_file)
                .unwrap();
            multi_gen
                .build()
                .unwrap()
    ...
    fn does_not_generate_shared_types_if_empty() {
        ...
        let tokens = MultiExpansion::new(vec![gen.expand().unwrap()])
            .expand_inplace()
            .to_string();
    ...
    fn can_filter_abigen() {
        ...
        let filtered = gen
            .clone()
            .with_filter(SelectContracts::default().add_name("MyGreeter2"));
        assert!(filtered.abigens.is_empty());

        let filtered = gen
            .clone()
            .with_filter(ExcludeContracts::default().add_name("MyGreeter"));
        assert!(filtered.abigens.is_empty());

        let filtered = gen
            .clone()
            .with_filter(SelectContracts::default().add_pattern("MyGreeter2"));
        assert!(filtered.abigens.is_empty());

        let filtered = gen
            .clone()
            .with_filter(ExcludeContracts::default().add_pattern("MyGreeter"));
        assert!(filtered.abigens.is_empty());
        ...
        let filtered = gen
            .clone()
            .with_filter(SelectContracts::default().add_pattern(".*Test"));
        assert_eq!(filtered.abigens.len(), 1);
        assert_eq!(filtered.abigens[0].contract_name, "MyGreeterTest");

        let filtered = gen
            .clone()
            .with_filter(ExcludeContracts::default().add_pattern(".*Test"));
        assert_eq!(filtered.abigens.len(), 1);
