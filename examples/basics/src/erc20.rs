...
pub mod mock_erc_20 {
    ...
    impl<M: ::revmup_client::RevmClient> MockErc20<M> {
        ...
        pub fn deploy<T: ::ethers::core::abi::Tokenize>(
            ...
            let abi = ethers::abi::Abi::from(MOCKERC20_ABI.clone());
            let encoded = match abi.constructor() {
                Some(c) => c.encode_input(MOCKERC20_BYTECODE.clone().to_vec().into(), &params)?,
    ...
    // @note EVENT EXPERIMENT
    impl<M: ::revmup_client::RevmClient> MockErc20<M> {
        pub fn parse_log(
            &self,
            name: &str,
            log: revm::primitives::Log,
        ) -> anyhow::Result<MockErc20Events> {
            let topics: Vec<::ethers::types::H256> =
                log.topics.iter().map(|x| x.clone().into()).collect();

            // @todo generate the following code via proc_...

            for (n, _) in &self.0.abi().events {
                println!("NAME: {:}", n)
            }

            match name {
                "Transfer" => {
                    let r: TransferFilter = self
                        .0
                        .decode_event::<TransferFilter>(name, topics, log.data.into())
                        .map_err(|_| anyhow::anyhow!("nope"))
                        .unwrap();
                    Ok(MockErc20Events::TransferFilter(r))
                }
                "Approval" => {
                    let r = self
                        .0
                        .decode_event::<ApprovalFilter>(name, topics, log.data.into())
                        .map_err(|_| anyhow::anyhow!("nope"))?;
                    Ok(MockErc20Events::ApprovalFilter(r))
                }
                _ => anyhow::bail!("ERROR"),
            }

            //let r = ::ethers::core::abi::RawLog {
            //    topics,
            //    data: log.data.into(),
            //};

            //use ::ethers::contract::EthLogDecode;
            //let v = MockErc20Events::decode_log(&r);
            //self.0
            //    .decode_event::<D>(name, topics, log.data.into())
            //    .map_err(|_| anyhow::anyhow!("nope"))
        }
    }
